...
func (s *Solr) EnterSearch_string(ctx *Search_stringContext) {
...
func (s *Solr) ExitSearch_string(ctx *Search_stringContext) {
...
func (s *Solr) ExitSearch_part(ctx *Search_partContext) {
...
func (s *Solr) ExitField_type(ctx *Field_typeContext) {
	log.Printf("ExitField_type: %s", ctx.GetText())
}

func (s *Solr) ExitField_query(ctx *Field_queryContext) {
	log.Printf("ExitField_query: %s", ctx.GetText())
}

// ExitBoolean_op is called when an OR or AND has just been parsed
func (s *Solr) ExitBoolean_op(ctx *Boolean_opContext) {
