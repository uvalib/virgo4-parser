...
func (eh *virgoErrorListener) LogError(msg string) {
	if eh.quiet == false {
		log.Printf("[V4] ERROR (%s): %s", eh.name, msg)
	}
...
func (eh *virgoErrorListener) LogWarning(msg string) {
	if eh.quiet == false {
		log.Printf("[V4] WARNING (%s): %s", eh.name, msg)
	}
...
func validate(src string, timeout int) (valid bool, errors string) {
	...
	defer func() {
		...
		if x := recover(); x != nil {
			...
			log.Printf("[V4] ERROR (recovered): %s", errors)
		} else {
		...
		log.Printf("[V4] Validation elapsed ms: %d", elapsedMS)
	}()

	log.Printf("[V4] Validate: %s", src)
